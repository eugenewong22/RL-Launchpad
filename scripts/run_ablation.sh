...
#
#   bash scripts/run_ablation.sh                     # all 5 arms, seed 0
#   bash scripts/run_ablation.sh --jobs 3            # 3 at a time
#   bash scripts/run_ablation.sh --resume            # skip finished arms
#   bash scripts/run_ablation.sh --keys "random_eps normalize_obs" --seeds "1 2"
#
# The --keys/--seeds form is for following up on the arms that carried a
# result. After the seed-0 sweep, only random_eps (collapses) and
# normalize_obs (2x slower) said anything; the other three landed inside the
# reported config's own seed band, so extra seeds there would only re-confirm
# "no effect". Four targeted runs beat ten uninformative ones.
#
...
for key in $KEYS; do
 for seed in $SEEDS; do
  # train.py appends _seed<N> when --seed is passed without --run-dir.
  out="results/ablate_${key}_seed${seed}"
  ...
  if [ "$RESUME" = 1 ] && [ -f "$out/progress.csv" ] \
    ...
    echo "skip ${key} seed ${seed}: already ran to 1M steps"
    continue
  fi

  echo "=== ablate $key seed $seed -> $out (log: logs/ablate_${key}_s${seed}.log) ==="
  $PY -m src.agent.train --config "configs/td3_her_push_ablate_${key}.yaml" \
      --seed "$seed" > "logs/ablate_${key}_s${seed}.log" 2>&1 &
